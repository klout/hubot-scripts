...
module.exports = (robot) ->
  robot.respond /elasticsearch status$/i, (msg) ->
    msg.http("http://localhost:9000/elasticsearch.json/status").auth(user, password).get() (err, res, body) ->
      msg.send body
  ...
  robot.respond /elasticsearch lock$/i, (msg) ->
    msg.http("http://localhost:9000/elasticsearch.json/lock/create").auth(user, password).get() (err, res, body) ->
      msg.send body

  robot.respond /elasticsearch unlock$/i, (msg) ->
    msg.http("http://localhost:9000/elasticsearch.json/lock/delete").auth(user, password).get() (err, res, body) ->
      msg.send body

  robot.respond /elasticsearch override ([^ ]+)$/i, (msg) ->
    overrideChoice = msg.match[1]
    msg.http("http://localhost:9000/elasticsearch.json/setOverride/#{overrideChoice}").auth(user, password).get() (err, res, body) ->
      msg.send body
